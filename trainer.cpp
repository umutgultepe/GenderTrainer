    ...
    char* filename_to_save, char* filename_to_load, double train_ratio ,int class_count)
{
    ...
    int ok = read_num_class_data( data_filename, 38, &data, &responses );
    int nsamples_all = 0, ntrain_samples = 0;
    int var_count;
    int i, j, k;
    double train_hr = 0, test_hr = 0;
    ...
    printf( "The database %s is loaded.\n", data_filename );
    nsamples_all = data->rows;
    ntrain_samples = (int)((double)nsamples_all*train_ratio);
    //ntrain_samples = (int)(nsamples_all);
    var_count = data->cols;
    ...
    else
    {
        ...
        boost.train( data, CV_ROW_SAMPLE, responses, 0, 0, var_type, 0,
